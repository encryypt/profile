...
setInterval(() => {
  http.get(`https://pacific-lunar-ring.glitch.me/`);
}, 280000);
...
msg.channel.send(`**:alarm_clock: You've already collected the daily credits, it will refresh in ( \`${timeObj.hours} Hours, ${timeObj.minutes} Minutes and ${timeObj.seconds} Seconds.\` )**`)}else{msg.channel.send(`**Done | You have Collected Your \`\`${amount}\`\` daily credits .**`);
